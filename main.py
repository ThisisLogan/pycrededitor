...
def search_laptop():
    ...
    if check_creds_name(string) == False:
        search_label.configure(text="Name does not exist")
    else:
...
def add_entry():
    ...
    if add_username.get() == '':
        ...
        if updated_count == 0:
            ...
            updated_count = updated_count + 1
        entry_data = {add_username.get():[{"password":add_password.get()},{"method": add_method.get()}]}
...
ttk.Button(root, text= "Search",width= 20, command= search_laptop).pack()
